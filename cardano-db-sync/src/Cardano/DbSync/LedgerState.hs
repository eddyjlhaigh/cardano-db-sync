...
import           Control.Monad.Extra (fromMaybeM, firstJustM, maybeM)
...
import           Ouroboros.Consensus.Ledger.Abstract (ledgerTipHash, ledgerTipPoint, ledgerTipSlot, tickThenReapply)
...
saveCurrentLedgerState env = do
    ...
    case mkLedgerStateFilename (leDir env) ledger of
      Origin -> return () -- we don't store genesis
...
loadState env point = case dbPointToFileName (leDir env) <$> getPoint point of
  At file -> maybeM (return $ Left file ) (return . Right) $ loadFile (topLevelConfig env) False file
